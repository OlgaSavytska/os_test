...
import images_proto from './templates/images_proto.hbs';

const parseUrl = 'https://api.giphy.com/v1/gifs/search';
const reqParams = '?api_key=O3iJIz4KNHqyrwfM88y7Abn9WA2607z0&q=happy&limit=8';

const imgBody = document.querySelector('.main_container')

fetch(parseUrl + reqParams).then(response => {
    console.log(response);
    return response.json()
}).then(data => {
    console.log(data); const markup = data.data.map(result => images_proto()).join('')
    console.log(markup)
})
